...
model.services.paymentFunction = async function (ID) {
  const services = await model.services.count({ where: { userID: ID } });

  const foundUser = await model.users.findOne({ where: ID });
  return { services: numberServices, foundUser: foundUser };
};
